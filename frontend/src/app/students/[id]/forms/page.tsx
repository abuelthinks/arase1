...
function UnifiedFormsViewerInner() {
    ...
    if (loading) {
        return <div className="p-8 text-center text-muted">Loading forms...</div>;
    }
                    ...
                    <div className="w-full md:w-72 border-b md:border-b-0 md:border-r border-line bg-app flex flex-col shrink-0">
                        ...
                        <div className="p-6 border-b border-line flex flex-col gap-4">
                            <div>
                                ...
                                <p className="text-xs text-faint mt-1 mb-0 uppercase tracking-wider font-semibold">Data Collection</p>
                            </div>
                                    ...
                                    {visibleTabs.filter(tab => ["parent_assessment", "multi_assessment"].includes(tab.id)).map((tab) => {
                                        ...
                                        return (
                                            <button 
                                                ...
                                                className={`w-full flex items-center justify-between text-left px-4 py-3 rounded-lg transition-all border ${isActive ? 'bg-indigo-50 border-indigo-200 shadow-sm relative' : 'border-transparent hover:bg-subtle-soft'}`}
                                            >
...
export default function UnifiedFormsViewer() {
    return (
        <Suspense fallback={<div className="p-8 text-center text-muted">Loading forms...</div>}>
